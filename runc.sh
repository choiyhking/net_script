...
if [ ! -z ${CPU} ]; then
	...
	do
		...
		sudo sh -c "sleep 1; pidstat -p \$(pgrep [n]etperf) 1 > ${HOST_RESULT_FILE} 2> /dev/null" &
  		PARENT_PID=$!
  		RESULT_FILE="${RESULT_FILE/$HOME/\/root}"
		sudo docker exec ${CONTAINER_NAME} /root/net_script/do_throughput.sh ${RESULT_FILE} ${REPEAT}
  		sudo sh -c "tail -n +3 ${HOST_RESULT_FILE} | awk '{print \$1, \$5, \$6, \$7, \$8, \$9, \$10, \$11}' > ${HOST_RESULT_FILE}"
